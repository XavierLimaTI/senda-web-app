  ...
  return (
    <div className="min-h-screen bg-[#F0EBE3]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-12">
            ...
            {favorites && favorites.length > 0 && (
              ...
              >
                ...
                {t('home.myFavorites')} ({favorites.length})
              </Link>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-6">
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              <div className="text-sm text-gray-600">{t('home.scheduledSessions')}</div>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm">
              <div className="text-3xl font-serif text-[#B2B8A3] mb-2">200+</div>
              <div className="text-sm text-gray-600">{t('home.verifiedTherapists')}</div>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm">
              <div className="text-3xl font-serif text-[#D99A8B] mb-2">4.8 <Star className="w-6 h-6 inline text-[#C8963E]" /></div>
              <div className="text-sm text-gray-600">{t('home.averageRating')}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {upcomingBookings.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          <div className="flex items-center gap-3 mb-8">
            <h2 className="text-3xl font-serif text-gray-900">{t('home.yourUpcomingSessions')}</h2>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">{t('home.with')} {booking.therapist.user.name}</p>
                    </div>
      ...
      {favorites && favorites.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          <div className="flex items-center gap-3 mb-8">
            <h2 className="text-3xl font-serif text-gray-900">{t('home.yourFavorites')}</h2>
                  ...
                  <div className="p-4">
                    ...
                    {fav.therapist.services[0] && (
                      <p className="text-xs text-gray-500 mt-2">
                        {t('home.startingFrom')} R$ {fav.therapist.services[0].price.toFixed(2).replace('.', ',')}
                      </p>
                    )}
                  </div>
                </div>
      ...
      <section className="bg-white py-16 md:py-24 mb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-serif text-gray-900 text-center mb-12">{t('home.howItWorks')}</h2>
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { icon: Search, title: t('home.stepExplore'), desc: t('home.stepExploreDesc') },
              { icon: Star, title: t('home.stepChoose'), desc: t('home.stepChooseDesc') },
              { icon: Calendar, title: t('home.stepSchedule'), desc: t('home.stepScheduleDesc') },
              { icon: Sparkles, title: t('home.stepTransform'), desc: t('home.stepTransformDesc') }
            ].map((step, idx) => (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div className="bg-gradient-to-r from-[#B2B8A3] to-[#C8963E] rounded-2xl p-12 text-white text-center">
          <h2 className="text-3xl font-serif mb-4">{t('home.readyToStart')}</h2>
          <p className="text-lg opacity-90 mb-6">{t('home.discoverAmazing')}</p>
          <Link
            href="/explore/therapists"
